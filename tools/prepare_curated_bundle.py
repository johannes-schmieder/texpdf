...
def download(url: str, destination: Path) -> None:
    ...
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "texpdf-bundle-builder/0.1", "Accept-Encoding": "identity"},
    )
    try:
...
def main() -> int:
    ...
    parser.add_argument("--info", type=Path, default=Path("bundle/generated/bundle-info.json"))
    parser.add_argument(
        "--cache-dir",
        type=Path,
    ...
    try:
        ...
        if args.trace is not None:
            trace_names = sorted(
                {line.strip() for line in args.trace.read_text(encoding="utf-8").splitlines() if line.strip()}
            )
        ...
        info = {
            ...
            "uncompressed_resource_bytes": sum(len(value) for value in resources.values()) + len(digest_bytes),
            "missing_trace_names": missing_from_index,
        }
