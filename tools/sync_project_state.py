...
def build_state(readiness_result: dict[str, Any]) -> dict[str, Any]:
    ...
    publication = read_json("release/publication.json")
...
def render_status(state: dict[str, Any]) -> str:
    ...
    qualification_bundle = state.get("qualification", {}).get("bundle", {})
    publication = state.get("publication", {})
    final_release = publication.get("final_release", {})
    ssc_submission = publication.get("ssc_submission", {})
    release_label = (
    ...
    lines = [
        ...
        "",
        (
            f"Version [{scope.get('candidate_version')}]({final_release.get('github_release_url')}) "
            "is the current stable GitHub release."
            if final_release.get("published") is True
            else f"The active target is a **public `{scope.get('candidate_version')}` {release_label}**."
        ),
        f"Qualified source: {code(scope.get('candidate_source_sha'))}.",
        (
            f"All {final_release.get('asset_count')} public release assets were downloaded and verified."
            if final_release.get("public_assets_verified") is True
            else "Public release assets have not yet been verified."
        ),
        (
            f"SSC submission was sent on {ssc_submission.get('submitted_at')}; publication is pending."
            if ssc_submission.get("submitted") is True
            and ssc_submission.get("publication_status") == "pending"
            else "SSC publication has not yet been submitted."
        ),
        "",
        f"Release qualification ready: **{str(readiness.get('candidate_ready')).lower()}**",
        "",
        ...
        f"| Release license-audit source | {code(licenses.get('source_sha'))} |",
    ...
    lines.extend(
        [
            ...
            f"{code(qualification_bundle.get('name'))} / "
            f"{code(qualification_bundle.get('version'))}, with ZIP SHA-256 "
            f"{code(qualification_bundle.get('zip_sha256'))}.",
            "",
            "## Memory evidence",
            "",
    ...
    if public_blockers:
        lines.extend(f"- Release blocker: `{blocker}`" for blocker in public_blockers)
    elif ssc_submission.get("publication_status") == "pending":
        lines.extend(
            [
