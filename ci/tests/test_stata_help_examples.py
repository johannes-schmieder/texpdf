...
class StataHelpExampleTests(unittest.TestCase):
    def test_repository_help_has_two_runnable_examples(self) -> None:
        text = HELP.read_text(encoding="utf-8")
        blocks = example_blocks(text)
        self.assertEqual(set(blocks), {"manual", "latexlog"})
        links = re.findall(
            r"\{stata texpdf_run ([a-z0-9_-]+) using texpdf\.sthlp, preserve:",
            text,
        )
        self.assertEqual(links, ["manual", "latexlog"])
        self.assertEqual(text.count("{* example_end}{...}"), 2)
